...
const ChatSection = () => {
  ...
  function WriteAiMessage(message) {
    ...
    return (
      <div className="overflow-auto bg-slate-950 text-white rounded-sm p-2">
        <Markdown
          children={messageObject.content}
          options={{
            overrides: {
  ...
  const renderMessageContent = (msg) => {
    if (msg.sender._id === "ai") {
      ...
      try {
        ...
        content = parsed.content || msg.message;
      } catch {
    ...
    return (
      <div className="overflow-auto bg-slate-200 text-gray-800 rounded-sm p-2">
  ...
  useEffect(() => {
    ...
    axios
     ...
     // 👉 VERY IMPORTANT: return a cleanup function
     return () => {
       // Remove the event listener when component unmounts / projectId changes
 ...
 useEffect(() => {
  ...
  return (
    <main className="min-h-[81.6vh] w-screen flex">
      <section className="left relative flex flex-col min-w-xl bg-slate-300">
          ...
          >
            {messages.map((msg, index) => (
              <div
                key={index}
                className={`${
                  msg.sender._id === "ai" ? "max-w-md" : "max-w-52"
                } ${
